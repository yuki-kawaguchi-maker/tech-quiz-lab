...
3. トップの「今日の20問を始める」を押すと、復習キューから最大5問(古い順)+
   未出題を中心とした新規(復習の不足分を補い常に合計20問になる)が自動編成されて
   出題される(1日1回の学習の型)。
...
5. トップには1000解答へのゲージ・連続日数(ストリーク、途切れても最長記録は残る)・
   「仕組み図鑑を見る」導線が常時表示される。図鑑では正解したことのある問題の
   key_conceptと図解が公開され、まだ正解していない問題は灰色の空きマスになる。
6. 進捗(累計成績・復習キュー・ゲージ・ストリーク・図鑑の解放状態)はブラウザの
   localStorageに保存される。端末・ブラウザを変えると引き継がれない点に注意する。
   トップ画面の「進捗をリセット」でいつでも初期化できる。
