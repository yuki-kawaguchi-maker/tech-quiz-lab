...
- `docs/curriculum-schema.md` — カリキュラム定義JSONのスキーマ定義(正本)
- `data/curriculum.json` — 最終ゴールと7ジャンル・全ユニット(作成済み/未作成を含む)の
  一覧。「学びの地図」画面の元データ
- `scripts/validate_questions.py` — 問題データのスキーマ適合チェック
- `scripts/validate_lessons.py` — レッスンデータのスキーマ適合チェック
- `scripts/validate_curriculum.py` — カリキュラム定義のスキーマ適合チェック
...
6. トップの「学びの地図を見る」から、最終ゴール(AX/DXコンサルとしてAIと一緒に自分で
   作り・提案し・運用できる)に向けた7ジャンル・全ユニットの縦型フィッシュボーン図を
   見られる。ユニット(小骨)は状態ごとに色分けされる(未作成=灰の点線、未学習=白、
   学習中=青、習得〈5問以上解答・正答率80%以上〉=緑)。今日のレッスンの対象ユニットには
   青い丸印が付く。ジャンル(大骨)をタップするとそのジャンルの教科書タブへ遷移する
   (まだレッスンが無いジャンルは「準備中」と表示される)。
7. トップには1000解答へのゲージ・連続日数(ストリーク、途切れても最長記録は残る)・
   「仕組み図鑑を見る」導線が常時表示される。図鑑では正解したことのある問題の
   key_conceptと図解、および学習済みユニットのカード(「(教科書)」と表示して区別)が
   公開され、まだ正解・学習していない項目は灰色の空きマスになる。
8. 進捗(累計成績・復習キュー・ゲージ・ストリーク・図鑑の解放状態・学習済みユニット)は
...
manifest.json・アイコンをキャッシュし、問題データ・レッスンデータ・カリキュラム定義
(`data/questions/*.json`・`data/lessons/*.json`・`data/curriculum.json`)は
